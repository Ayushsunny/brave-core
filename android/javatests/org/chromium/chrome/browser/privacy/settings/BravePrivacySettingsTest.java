    ...
    public void testPrivacySandboxDefaultIsDisabled() {
        TestThreadUtils.runOnUiThreadBlocking(() -> {
            if (PrivacySandboxBridge.isPrivacySandboxSettingsFunctional()) {
                assertNotEquals(null, mFragment.findPreference(PREF_PRIVACY_SANDBOX));
                assertEquals(mActivityTestRule.getActivity().getResources().getString(
                                     R.string.privacy_sandbox_status_disabled),
                        mFragment.findPreference(PREF_PRIVACY_SANDBOX).getSummary());
            } else {
                assertEquals(null, mFragment.findPreference(PREF_PRIVACY_SANDBOX));
            }
        });
