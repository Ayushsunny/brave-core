...
void CreateRequestForFileList(ResourceRequestGetter request_callback,
    content::BrowserContext::BlobContextGetter storage,
    const base::FilePath& folder_path, std::vector<ImportFileInfo> files) {
  std::string mime_boundary = net::GenerateMimeMultipartBoundary();
  auto blob_builder_callback =
      base::BindOnce(&BuildBlobWithFolder, folder_path.DirName(),
